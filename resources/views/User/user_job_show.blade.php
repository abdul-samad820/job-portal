        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <div class="d-flex justify-content-between align-items-center mb-4">
                    ...
                    <!-- Sort form: preserves current filters via hidden inputs -->
                    <form id="sortForm" method="GET" action="{{ route('user.jobs.filter') }}" class="m-0 p-0">
                        <input type="hidden" name="search" value="{{ request('search') }}">
                        <input type="hidden" name="category" value="{{ request('category') }}">
                        <input type="hidden" name="role" value="{{ request('role') }}">
                        <input type="hidden" name="min_salary" id="sortMinSalary" value="{{ request('min_salary') }}">
                        <input type="hidden" name="max_salary" id="sortMaxSalary" value="{{ request('max_salary') }}">

                        @foreach ((array) request('job_type', []) as $jt)
                            <input type="hidden" name="job_type[]" value="{{ $jt }}">
                        @endforeach
                        @foreach ((array) request('experience', []) as $ex)
                            <input type="hidden" name="experience[]" value="{{ $ex }}">
                        @endforeach

                        <select name="sort" class="form-control" style="max-width:220px;"
                            onchange="document.getElementById('sortForm').submit();">
                            <option value="latest" {{ request('sort') == 'latest' ? 'selected' : '' }}>Latest</option>
                            <option value="experience_asc" {{ request('sort') == 'experience_asc' ? 'selected' : '' }}>
                                Experience (Fresher → Exp)</option>
                            <option value="experience_desc" {{ request('sort') == 'experience_desc' ? 'selected' : '' }}>
                                Experience (Exp → Fresher)</option>
                    ...
                    <div class="card shadow-sm border-0 rounded-4 mb-4 position-relative">
                        <div class="card-body p-4">
                            <div class="d-flex justify-content-between">
                                <h5 class="text-dark mb-0">{{ $job->title }}</h5>
                            ...
                            <p class="text-muted small mt-2">{{ $job->admin->company_name }}</p>
