...
public function admin_applications(Request $request) {
    ...
    $applications = JobApplication::with(['user.profile','job','updatedBy'
])->whereIn('job_id', $jobIds)->when($request->job, fn($q) =>$q->where('job_id', $request->job)
...
public function applyFromSaved($id) {
    ...
    SavedJob::where('user_id', $userId)->where('job_id', $id)->delete();
...
public function jobapplications(Request $request) {
    ...
    ->whereIn('job_id', $jobIds)->orderBy('created_at','desc')->paginate(10);
